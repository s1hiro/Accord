...
function showDmPartners(username) {
  ...
  db.child(`dms/${username}`)
    .once("value")
    .then((snap) => {
      const partners = snap.val() ? Object.keys(snap.val()) : [];
      if (partners.length === 0) {
        adminDmList.innerHTML =
          '<div class="no-dms">No DM partners found.</div>';
        return;
      }
...
function showMessages(user, partner) {
  ...
  Promise.all(paths.map((p) => db.child(p).once("value"))).then(([s1, s2]) => {
    ...
    msgs.sort(
      (a, b) => (a.val.timestampNumeric || 0) - (b.val.timestampNumeric || 0)
    );

    if (msgs.length === 0) {
      adminMessagesList.innerHTML =
        '<div class="no-dms ">No messages found.</div>';
      return;
    }
...
function deleteUserFully(username) {
  ...
  // 1️⃣ Delete this user's own data (profile, and dms)
  db.child("users").child(username).remove();
  db.child("dms").child(username).remove();

  // 2️⃣ Go through every user and remove references to this username
  db.child("users")
    .once("value")
    .then((snap) => {
      const allUsers = snap.val() || {};

      Object.keys(allUsers).forEach((other) => {
        db.child(`users/${other}/friends/${username}`).remove();
        db.child(`dms/${other}/${username}`).remove();
      });

      // 3️⃣ Clean up UI after deletion
      adminDmList.innerHTML = "";
      adminMessagesList.innerHTML = "";
